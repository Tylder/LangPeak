...
import {QuestionData} from '../db/lesson';

function _createControls(questions: QuestionData[],
                         questionValidators?: ValidatorFn | ValidatorFn[] | null): { [key: string]: (Question | QuestionGroup) } {

  /**
   * Takes an array of questionData and validators and returns an object with the id of the questions as key and the question as value
  ...
  questions.forEach((qData, index) => {
    ...
    if (!('questions' in qData) || qData?.questions.length === 1) {
      ...
      tempControls[qData.id] = new QuestionGroup(qData.questions, qData, questionValidators);
    }
  });
  return tempControls;
}
...
export class QuestionGroup extends FormGroup {
  ...
  constructor(questions: QuestionData[], _questionData?: QuestionData,
    ...
    super(_createControls(questions, questionValidators));
  ...
  addQuestions(questions: QuestionData[], questionValidators?: ValidatorFn | ValidatorFn[] | null) {
    ...
    const tempControls = _createControls(questions, questionValidators);
  ...
  getQuestionsAndGroups(): (Question | QuestionGroup)[] {
    /**
     * Returns all questions and groups in the group, its like a one step down ls
     */
  ...
  getQuestionsDeep(): (Question | undefined)[] {
    ...
    /**
     * Similar to getQuestions but goes through question groups as well to get all questions,
  ...
  setQuestionValidators(newValidator: ValidatorFn | ValidatorFn[] | null): void {
    /**
     *  Sets the Validators on the each control in the group, only on the top level questions, not in each group
     */
